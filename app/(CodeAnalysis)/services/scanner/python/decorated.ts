...
export function scanDecoratedFunction(node:any) :any {
    
    for (const node_child of node.children) {
     
        if (node_child.type === "function_definition") {
            // Get the name of the function
            const function_name = node_child.childForFieldName("name").text;
            const start_point=node_child.startPosition
            const end_point=node_child.endPosition
            // Get the input arguments of the function
            const function_args = [];
            for (const child of node_child.childForFieldName("parameters").children) {
            ...
            // Initialize a list of try catch statements in the function
            
            // Scan the body of the function
            for (const child of node_child.childForFieldName("body").children) {
